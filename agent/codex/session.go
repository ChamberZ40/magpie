...
func prependCodexPromptPreamble(prompt string, preamble string) string {
	...
	if prompt == "" {
		return "Before answering, follow these project-level instructions for this magpie session. They are not user content.\n\n" + preamble
	}
	return "Before answering, follow these project-level instructions for this magpie session. They are not user content.\n\n" + preamble + "\n\n---\n\nUser message:\n" + prompt
}
...
func (cs *codexSession) stageImages(prompt string, images []core.ImageAttachment) (string, []string, error) {
	...
	imgDir := filepath.Join(appid.WorkspaceDir(cs.workDir), "images")
...
func (cs *codexSession) buildExecArgs(prompt string, imagePaths []string) []string {
	...
	// — and the user would silently lose their session on every magpie
	// restart / idle reset.
	//
...
func loadCodexRuntimeConfig(ctx context.Context, workDir string, extraEnv []string) (string, string, error) {
	...
	if err := rpcRequestOverIO(stdin, reader, nextID, "initialize", map[string]any{
		"clientInfo": map[string]any{
			"name":    "magpie-codex-runtime-config",
			"title":   "Magpie Codex Runtime Config",
			"version": "0.1.0",
		},
	}, nil); err != nil {
		return "", "", err
