...
The Bridge Protocol allows **external platform adapters** written in any programming language to connect to magpie at runtime via WebSocket. This eliminates the requirement to write Go code and recompile the binary for every new platform integration.
...
The `BridgePlatform` is a built-in platform inside magpie that:
...
| `reply_ctx` | string | yes | Opaque context string the adapter needs to route replies back. magpie echoes this in every reply. |
...
Keepalive. magpie responds with `pong`.
...
Send a structured card to the user. Only sent if the adapter declared `"card"` capability; otherwise magpie falls back to `reply` with `card.RenderText()`.
...
If a capability is not declared, magpie will automatically degrade:
...
3. Resume normal operation — magpie maintains session state independently of the connection.

### Message Ordering

Messages within a single WebSocket connection are ordered. magpie processes adapter messages sequentially per session key.

### Timeouts

- **Ping interval**: Adapters should send `ping` at least every 30 seconds.
- **Connection timeout**: magpie closes idle connections after 90 seconds without a ping.
- **Reply timeout**: If an agent takes too long, magpie may send an error reply. The adapter does not need to handle this specially.
...
1. **Keep it stateless** — the adapter should be a thin translation layer. All session state lives in magpie.
...
The protocol version is declared in the `register` message via `metadata.protocol_version`. The current version is `1`. magpie will reject connections with incompatible versions and respond with a `register_ack` containing an error.
