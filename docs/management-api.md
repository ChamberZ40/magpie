# magpie Management API Specification
...
The magpie Management API is an HTTP-based REST API that enables external applications (web dashboards, TUI clients, GUI desktop apps, Mac tray apps) to manage and monitor magpie instances. It complements the existing internal Unix socket API by providing a network-accessible, token-authenticated interface suitable for remote and local management tools.
...
|-----------------------|----------|--------------------------------------------------|
| `version`             | string   | magpie version (e.g. `v1.2.0`)              |
    ...
    "entries": [
      {
        ...
        "attrs": {"socket": "/home/user/.magpie/run/api.sock"}
      }
    ]
  }
}
