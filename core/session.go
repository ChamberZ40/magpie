...
type Session struct {
	...
	// magpie process restart does not silently drop a user's
...
// KnownAgentSessionIDs returns the set of agent session IDs tracked by magpie.
// This is used to filter agent.ListSessions() output to only sessions owned by
// magpie, excluding sessions created by external CLI usage in the same work_dir.
