...
// `magpie cron add --silent` flag. The flag is parsed in runCronAdd
// (cmd/magpie/cron.go) and sets body["silent"] = true on the /cron/add
// request; if printCronAddUsage ever drifts and stops mentioning it, users
// who run `magpie cron add --help` will not discover the feature even
...
func TestPrintCronAddUsage_DocumentsSilent(t *testing.T) {
	...
	if !strings.Contains(help, "magpie cron add --cron \"0 9 * * *\" --prompt \"Daily standup reminder\" --silent") {
