...
func runSessions(args []string) {
	...
	case "show":
		...
		if id == "" {
			...
			fmt.Fprintln(os.Stderr, "Usage: magpie sessions show <session-id> [-n N]")
			os.Exit(1)
		}
...
func runSessionsShow(dataDir, id string, limit int) {
	...
	if record == nil {
		...
		fmt.Fprintln(os.Stderr, "Use 'magpie sessions list' to see available sessions.")
		os.Exit(1)
	}
...
func printSessionsUsage() {
	fmt.Println(`Usage: magpie sessions [command] [options]
...
Examples:
  magpie sessions                           Interactive TUI browser
  magpie sessions list                      List all sessions
  magpie sessions show "mybot:s1"           Show all messages in session
  magpie sessions show "#1" -n 20           Show last 20 messages of first session
  magpie sessions prune                     Remove empty duplicate sessions
  magpie sessions prune --empty             Same as above, explicit form
  magpie sessions prune --merge             Merge duplicates, keeping most recent
  magpie sessions prune mybot --merge       Prune specific project`)
}
