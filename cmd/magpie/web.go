...
func runWeb(args []string) {
	...
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "Config file not found: %s\nRun magpie first to create a default config.\n", configPath)
		os.Exit(1)
	}

	// Use LoadPermissive so `magpie web` works even before any platforms
	...
	if !mgmtEnabled {
		...
		fmt.Println("Restart magpie for the changes to take effect.")
	}
	...
	if err := openBrowser(loginURL); err != nil {
		...
		fmt.Printf("\nNote: make sure magpie is running (it hosts the web admin on port %d).\n", port)
	}
}
