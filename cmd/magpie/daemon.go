...
func daemonInstall(args []string) {
	...
	fmt.Println("magpie daemon installed and started.")
	fmt.Println()
	...
	fmt.Println("  magpie daemon status    - Check status")
	fmt.Println("  magpie daemon logs -f   - Follow logs")
	fmt.Println("  magpie daemon restart   - Restart")
	fmt.Println("  magpie daemon stop      - Stop")
	fmt.Println("  magpie daemon uninstall - Remove")
	...
	if strings.Contains(mgr.Platform(), "user") {
		...
		if !enabled {
			...
			fmt.Println("   magpie will stop when your last login session ends (e.g., SSH disconnect).")
...
func parseDaemonInstallArgs(args []string) (daemon.Config, bool, error) {
	...
	// Env-based opt-out: MAGPIE_DAEMON_NO_CAPTURE_SECRETS=1 / true / yes / on
	...
	if isTruthyEnv(appid.Getenv("DAEMON_NO_CAPTURE_SECRETS")) {
...
func daemonUninstall() {
	...
	daemon.RemoveMeta()
	fmt.Println("magpie daemon uninstalled.")
}
...
func daemonStart() {
	...
	fmt.Println("magpie daemon started.")
}
...
func daemonStop() {
	...
	fmt.Println("magpie daemon stopped.")
}
...
func daemonRestart(args []string) {
	...
	fmt.Println("magpie daemon restarted.")
}
...
func requireInstalled(mgr daemon.Manager) {
	...
	if st == nil || !st.Installed {
		...
		fmt.Fprintln(os.Stderr, "  magpie daemon install --work-dir /path/to/config-dir")
		os.Exit(1)
	}
}
...
func daemonStatus() {
	...
	fmt.Println("magpie daemon status")
	fmt.Println()
	...
	if !st.Installed {
		...
		fmt.Printf("  Platform:  %s\n", st.Platform)
		fmt.Println()
		fmt.Println("  Run: magpie daemon install")
		return
	}
...
func printDaemonUsage() {
	fmt.Println(`Usage: magpie daemon <command> [flags]
...
Install flags:
  ...
  --log-file PATH       Log file path (default: ~/.magpie/logs/magpie.log)
  ...
  --no-capture-secrets  Do not capture config.toml ${ENV} placeholders into
                        ...
                        MAGPIE_DAEMON_NO_CAPTURE_SECRETS=1 in the environment.
