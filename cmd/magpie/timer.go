...
func runTimerInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: timer ID is required")
		fmt.Fprintln(os.Stderr, "Usage: magpie timer info <id>")
		os.Exit(1)
	}
...
func printTimerUsage() {
	fmt.Println(`Usage: magpie timer <command> [options]
...
Run 'magpie timer <command> --help' for details.`)
}

func printTimerAddUsage() {
	fmt.Println(`Usage: magpie timer add [options] <delay> <prompt>
