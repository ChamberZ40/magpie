...
func runAgentSID(args []string) {
	...
	if project == "" {
		fmt.Fprintln(os.Stderr, "Error: project is required (set MAGPIE_PROJECT env or use --project)")
		os.Exit(1)
	}
	if sessionKey == "" {
		fmt.Fprintln(os.Stderr, "Error: session key is required (set MAGPIE_SESSION_KEY env or use --session-key)")
		os.Exit(1)
	}
...
func printAgentSIDUsage() {
	fmt.Println(`Usage: magpie agent-sid [options]

Print the agent session ID (e.g. Claude Code, Codex, Gemini CLI) for the
current session. This is the ID used for --resume.

The command reads from the persisted session file; no running magpie
instance is required.

Options:
  -p, --project <name>       Project name (auto-detected from MAGPIE_PROJECT env)
  -s, --session-key <key>    Session key  (auto-detected from MAGPIE_SESSION_KEY env)
      --data-dir <path>      Data directory (default: ~/.magpie)
  -h, --help                 Show this help

Examples:
  magpie agent-sid                         Auto-detect from env (inside a session)
  magpie agent-sid -p mybot -s "discord:123:456"`)
}
