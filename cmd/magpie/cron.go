...
func runCronAdd(args []string) {
	...
	// Fallback to env vars (set by magpie when spawning agent)
	if project == "" {
		project = appid.Getenv("PROJECT")
	}
	if sessionKey == "" {
		sessionKey = appid.Getenv("SESSION_KEY")
	}
	...
	if _, err := os.Stat(sockPath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "Error: magpie is not running (socket not found: %s)\n", sockPath)
		os.Exit(1)
	}
...
func runCronInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Usage: magpie cron info <id> [field]")
		fmt.Fprintln(os.Stderr, "Use 'magpie cron list' to see all task IDs.")
		os.Exit(1)
	}
...
func runCronEdit(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Run 'magpie cron edit --help' for usage.")
		os.Exit(1)
	}
	if field == "" {
...
// documented as a bool in printCronEditUsage, so `magpie cron edit <id>
...
func printCronUsage() {
	fmt.Println(`Usage: magpie cron <command> [options]
...
Run 'magpie cron <command> --help' for details.`)
}

func printCronAddUsage() {
	fmt.Println(`Usage: magpie cron add [options] [<min> <hour> <day> <month> <weekday> <prompt>]

Create a new scheduled task (agent prompt or shell command).

Options:
  -p, --project <name>       Target project (auto-detected from MAGPIE_PROJECT env)
  -s, --session-key <key>    Target session (auto-detected from MAGPIE_SESSION_KEY env)
  -c, --cron <expr>          Cron expression, e.g. "0 6 * * *"
      ...
      --data-dir <path>      Data directory (default: ~/.magpie)
  -h, --help                 Show this help

Examples:
  magpie cron add --cron "0 6 * * *" --prompt "Collect GitHub trending data" --desc "Daily Trending"
  magpie cron add --cron "*/30 * * * *" --exec "df -h" --desc "Disk usage check"
  magpie cron add --cron "0 9 * * *" --prompt "Daily standup reminder" --silent
  magpie cron add 0 6 * * * Collect GitHub trending data and send me a summary`)
}

func printCronExecUsage() {
	fmt.Println(`Usage: magpie cron exec <id> [options]
...
Example:
  ...
  magpie cron run abc123`)
}

func printCronEditUsage() {
	fmt.Println(`Usage: magpie cron edit <id> <field> <value> [options]
...
Examples:
  ...
  magpie cron edit abc123 description "Daily standup reminder"
  magpie cron edit abc123 timeout_mins 60
  magpie cron edit abc123 mute true`)
}
