...
func runRelaySend(args []string) {
	...
	if sessionKey == "" {
		fmt.Fprintln(os.Stderr, "Error: session key is required (set MAGPIE_SESSION_KEY or use --session-key)")
		os.Exit(1)
	}
...
func printRelayUsage() {
	fmt.Println(`Usage: magpie relay <command> [options]

Commands:
  send      Send a message to another bot via relay

Run 'magpie relay <command> --help' for details.`)
}

func printRelaySendUsage() {
	fmt.Println(`Usage: magpie relay send [options] [<target_project> <message>]

Send a message to another bot and wait for the response.

Options:
  -f, --from <project>       Source project (auto-detected from MAGPIE_PROJECT env)
  -t, --to <project>         Target bot project name
  -s, --session-key <key>    Session key (auto-detected from MAGPIE_SESSION_KEY env)
  -m, --message <text>       Message to send
      --data-dir <path>      Data directory (default: ~/.magpie)
  -h, --help                 Show this help

Examples:
  magpie relay send --to claude-bot "What's the weather today?"
  magpie relay send claude-bot What is the weather today`)
}
