...
func printProviderUsage() {
	fmt.Println(`Usage: magpie provider <command> [options]
...
Examples:
  magpie provider add --project my-backend --name relay --api-key sk-xxx
  magpie provider add --project my-backend --name bedrock --env CLAUDE_CODE_USE_BEDROCK=1,AWS_PROFILE=bedrock
  magpie provider list --project my-backend
  magpie provider remove --project my-backend --name relay
  magpie provider import --project my-backend
  magpie provider presets
  magpie provider global list
  magpie provider global add --name minimaxi --api-key sk-xxx --base-url https://api.minimaxi.chat/v1`)
}
...
func runProviderGlobal(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, `Usage: magpie provider global <command>
...
func runGlobalProviderList(args []string) {
	...
	if len(providers) == 0 {
		...
		fmt.Println("\nAdd one with: magpie provider global add --name <name> --api-key <key>")
		return
	}
