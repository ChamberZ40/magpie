...
func parseSendArgs(args []string) (core.SendRequest, string, error) {
	...
	if req.Project == "" {
		req.Project = strings.TrimSpace(appid.Getenv("PROJECT"))
	}
	if req.SessionKey == "" {
		req.SessionKey = strings.TrimSpace(appid.Getenv("SESSION_KEY"))
	}
	if req.Message == "" {
	...
	// transcode for mp3 → opus). See magpie internal task
	// t-20260615-cqjbk1.
	req.Files = files
...
// daemon resolves it — so the standalone `magpie send` subcommand (a
...
func resolveSocketPath(dataDir string) string {
	...
	// The daemon may have been handed a custom data_dir through the environment.
	if envDataDir := strings.TrimSpace(appid.Getenv("DATA_DIR")); envDataDir != "" {
		return filepath.Join(envDataDir, "run", "api.sock")
	}
	if dir, err := appid.HomeDir(); err == nil {
		return filepath.Join(dir, "run", "api.sock")
	}
	return filepath.Join(appid.DirIn("."), "run", "api.sock")
}

func printSendUsage() {
	fmt.Println(`Usage: magpie send [options] <message>
       magpie send [options] -m <message>
       magpie send [options] --stdin < file
       magpie send [options] --image <path>
       magpie send [options] --file <path>
       magpie send [options] --audio <path>
       magpie send [options] --video <path>
       magpie send [options] --tts <text>
       echo "msg" | magpie send [options] --stdin

Send a message, attachment, or synthesized voice message to an active magpie session.
...
Examples:
  magpie send "Daily summary: ..."
  magpie send -m "Build completed successfully"
  magpie send --message "Chart generated" --image /tmp/chart.png
  magpie send --file /tmp/report.pdf
  magpie send --video /tmp/demo.mp4
  magpie send --audio /tmp/voice.opus
  magpie send --tts "Hello from magpie"
  magpie send --stdin <<'EOF'
