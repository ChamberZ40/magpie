...
func runDoctor(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: magpie doctor <subcommand>")
...
func runDoctorUserIsolation(args []string) {
	...
	outPath := fs.String("out", "", "path to write JSON report (default: ~/.magpie/audits/<timestamp>-<project>.json per project)")
...
// defaultAuditDir returns ~/.magpie/audits for the supervisor user.
func defaultAuditDir() (string, error) {
	dir, err := appid.HomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(dir, "audits"), nil
}
